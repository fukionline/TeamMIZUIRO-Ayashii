...
sub disp {
	...
	if ($email) { print "　Poster：<b><a href='mailto:$email'>$name</a></b>\n"; }
...
EOF
	...
	if (($imgview == 0 || $flag eq 'search2' || $flag eq 'thread' || ! (-e "$imgdir/$img")) && $img) { 
		if (-e "$imgdir/$img") {
			print "<font color='#ff69b4'>Upload Image：</font><a href='$imgdir/$img' target='_blank' rel='noopener noreferrer'>$img</a>\n";
		}
		else {
			print "<font color='#ff69b4'>Upload Image：</font><font color='#$resc'>$img (Deleted)</font>\n";
		}
	}
	elsif ($img) {
...
sub mbdisp {
	...
	if ($img) { 
		if(-e "$imgdir/$img"){
			print "<span class='c3'>Image ：</span><a href='$imgdir/$img' target='_blank' rel='noopener noreferrer'>$img</a>\n";
		}
		else{
			print "<span class='c2'>Image ：$img (Deleted)</span>\n";
		}
	}
	print "<br>\n";
...
sub pushlog {
	...
	if ($email) { print LOG qq!　Poster：<b><a href="mailto:$email">$name</a></b>\n!; }
	else { print LOG qq!　Poster：<font color="#$subjc"><b>$name</b></font>\n!; }
...
EOF
	...
	if ($found) {
		...
		# hpage関連のURLには./$scriptnameを使用
		print "<font color='#ff69b4'></font>※URLs in quoted text will not be automatically converted to links regardless of the checkbox status. Uploaded images will not be quoted.<br><br>\n\n";
...
EOF
	...
	if ($rankkey) {
		print "<hr>Number of submissions this month：";
